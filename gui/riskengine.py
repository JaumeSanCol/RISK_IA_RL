...
def logai(string):
    ...
    print(currentplayer.name + ": " + string, file=verbosefile)
...
class Player(object):
    ...
    def __init__(self, name):
        ...
        self.economy=ECON_START
        self.happiness=HAPP_START
        self.development=DEVP_START
...
def removeplayer(player, conqueror):
    ...
    if len(players) == 1: #game over
        riskgui.won_game(list(players.values())[0].name)
        setphase("PostGame")
...
def handle_place(terr, button):
    ...
    if currentplayer.freeArmies > 0:
        ...
        setphase("Attack")
        print("Attacking", file=verbosefile)
        return 0
...
def startgame():
    ...
    playerorder = list(players.values())
...
def load_game(filename):
    ...
    while savefile.readline() == "player\n":
        ...
        pleconomy = int(savefile.readline().strip())
        plhappiness = int(savefile.readline().strip())
        pldevelopment = int(savefile.readline().strip())
        ...
        players[plname].economy = pleconomy
        players[plname].happiness= plhappiness
        players[plname].development = pldevelopment
